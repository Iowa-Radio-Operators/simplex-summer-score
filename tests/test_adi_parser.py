...
def test_parse_digital_record():
    from app.adi_parser import parse_adi_file
    text = _adi_text([{"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240615", "TIME_ON": "200000", "MODE": "FT8", "FREQ": "146.520", "DIGITAL_MODE": "FT4/8"}])
    result = parse_adi_file(text)
...
def test_duplicate_detection_same_call_date_time_mode():
    ...
    from app.adi_parser import parse_adi_file
    recs = [
        {"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "FM", "FREQ": "146.520"},
        {"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "FM", "FREQ": "146.520"},
    ]
    result = parse_adi_file(_adi_text(recs))
...
def test_no_false_duplicate_different_callsign():
    ...
    recs = [
        {"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "FM", "FREQ": "146.520"},
        {"MY_CALL": "W0ABC", "CALL": "N2DEF", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "FM", "FREQ": "146.520"},
    ]
    result = parse_adi_file(_adi_text(recs))
...
def test_no_false_duplicate_different_frequency():
    ...
    recs = [
        {"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "FM", "FREQ": "146.520"},
        {"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "FM", "FREQ": "146.580"},
    ]
    result = parse_adi_file(_adi_text(recs))
...
def test_no_false_duplicate_different_mode():
    ...
    recs = [
        {"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "FM", "FREQ": "146.520"},
        {"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "USB", "FREQ": "146.520"},
    ]
    result = parse_adi_file(_adi_text(recs))
...
def test_no_false_duplicate_different_date():
    ...
    recs = [
        {"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "FM", "FREQ": "146.520"},
        {"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240616", "TIME_ON": "143000", "MODE": "FM", "FREQ": "146.520"},
    ]
    result = parse_adi_file(_adi_text(recs))
...
def test_no_false_duplicate_different_time():
    ...
    recs = [
        {"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "FM", "FREQ": "146.520"},
        {"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240615", "TIME_ON": "143100", "MODE": "FM", "FREQ": "146.520"},
    ]
    result = parse_adi_file(_adi_text(recs))
...
def test_duplicate_detection_with_call_fallback():
    ...
    from app.adi_parser import parse_adi_file
    recs = [
        {"CALL": "K1ABC", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "FM", "FREQ": "146.520"},
        {"CALL": "K1ABC", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "FM", "FREQ": "146.520"},
    ]
...
def test_adi_batch_rejects_duplicate_records(client, app):
    ...
    with app.app_context():
        count = Submission.query.filter_by(contact_call="K1XYZ").count()
        assert count == 2, f"Expected 2 submissions for K1XYZ (seed + first batch), got {count}"
...
def test_adi_batch_rejects_duplicate_records_no_my_call(client, app):
    ...
    with app.app_context():
        count = Submission.query.filter_by(contact_call="K1ABC").count()
        assert count == 1, f"Expected 1 submission for K1ABC, got {count}"
...
def test_adif_master_format_parses_all_records():
    ...
    records = [
        '<CALL:5>K4JJQ <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>032800 <STATION_CALLSIGN:6>KC0UJC <OPERATOR:6>KC0UJC <MY_GRIDSQUARE:6>EN32ia <POTA:6>PARK-1 <COMMENT:25>Test contact 1<EOR>',
        '<CALL:6>WD8JLD <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>032900 <STATION_CALLSIGN:6>KC0UJC <OPERATOR:6>KC0UJC <MY_GRIDSQUARE:6>EN32ia <COMMENT:25>Test contact 2<EOR>',
        '<CALL:4>NF0T <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>033300 <STATION_CALLSIGN:6>KC0UJC <OPERATOR:6>KC0UJC <MY_GRIDSQUARE:6>EN32ia <COMMENT:25>Test contact 3<EOR>',
    ]
    ...
    assert result.records[0].contact_call == "K4JJQ"
    assert result.records[1].contact_call == "WD8JLD"
    assert result.records[2].contact_call == "NF0T"
...
def test_adif_master_format_with_cw_excluded():
    ...
    records = [
        '<CALL:5>K4JJQ <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>032800 <STATION_CALLSIGN:6>KC0UJC <OPERATOR:6>KC0UJC <MY_GRIDSQUARE:6>EN32ia <COMMENT:25>Contact 1<EOR>',
        '<CALL:4>W0JW <MODE:2>CW <BAND:2>6m <FREQ:6>50.100 <QSO_DATE:8>20260609 <TIME_ON:6>041259 <STATION_CALLSIGN:6>KC0UJC <OPERATOR:6>KC0UJC <MY_GRIDSQUARE:6>EN32ia <COMMENT:25>CW contact<EOR>',
        '<CALL:5>W6GRE <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>033700 <STATION_CALLSIGN:6>KC0UJC <OPERATOR:6>KC0UJC <MY_GRIDSQUARE:6>EN32ia <COMMENT:25>Contact 3<EOR>',
    ]
    ...
    assert result.records[0].contact_call == "K4JJQ"
    assert result.records[1].contact_call == "W6GRE"
    assert len(result.excluded_modes) == 1
    assert result.excluded_modes[0]["mode"] == "CW"
...
def test_adif_master_format_all_cw_excluded():
    ...
    records = [
        '<CALL:4>K0KT <MODE:2>CW <BAND:2>6m <FREQ:6>50.100 <QSO_DATE:8>20260609 <TIME_ON:6>034900 <STATION_CALLSIGN:6>KC0UJC <OPERATOR:6>KC0UJC <MY_GRIDSQUARE:6>EN32ia <COMMENT:25>CW contact 1<EOR>',
        '<CALL:4>W0JW <MODE:2>CW <BAND:2>6m <FREQ:6>50.100 <QSO_DATE:8>20260609 <TIME_ON:6>041259 <STATION_CALLSIGN:6>KC0UJC <OPERATOR:6>KC0UJC <MY_GRIDSQUARE:6>EN32ia <COMMENT:25>CW contact 2<EOR>',
    ]
...
def test_adif_master_format_preserves_pot_a_field():
    ...
    records = [
        '<CALL:5>K4JJQ <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>032800 <MY_POTA_REF:7>US-9913 <COMMENT:25>POTA contact<EOR>',
    ]
...
def test_adif_master_format_with_many_records():
    ...
    for i in range(20):
        call = f"W{i:04d}"
        records.append(f'<CALL:{len(call)}>{call} <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>{i:02d}0000 <STATION_CALLSIGN:6>KC0UJC <OPERATOR:6>KC0UJC <MY_GRIDSQUARE:6>EN32ia <COMMENT:5>Contact {i}<EOR>')
...
def test_cw_excluded_but_other_records_remain():
    ...
    recs = [
        {"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "FM"},
        {"MY_CALL": "W0ABC", "CALL": "N2DEF", "QSO_DATE": "20240615", "TIME_ON": "150000", "MODE": "CW"},
        {"MY_CALL": "W0ABC", "CALL": "M3GHI", "QSO_DATE": "20240615", "TIME_ON": "160000", "MODE": "USB"},
    ]
    ...
    assert len(result.excluded_modes) == 1
    assert result.excluded_modes[0]["call"] == "N2DEF"
...
def test_excluded_modes_warning_in_preview(client, app):
    ...
    _seed_subs(app)
    recs = [
        {"MY_CALL": "W0ABC", "CALL": "K1XYZ", "QSO_DATE": "20240615", "TIME_ON": "143000", "MODE": "CW"},
    ]
...
def test_adif_master_format_with_eoh_header():
    ...
    content = (
        ...
        "\n"
        '<CALL:5>K4JJQ <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>032800 <EOR>\n'
        '<CALL:6>WD8JLD <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>032900 <EOR>\n'
    )
...
def test_adif_master_format_preserves_all_fields():
    ...
    content = (
        ...
        '<CALL:5>K4JJQ <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>032800 '
        '<STATION_CALLSIGN:6>KC0UJC <OPERATOR:6>KC0UJC <GRIDSQUARE:4>FM18 <MY_GRIDSQUARE:6>EN32ia '
        '<NAME:13>John J. Quinn <QTH:9>Warrenton <STATE:2>VA <COMMENT:25>Test contact<EOR>\n'
    )
    ...
    assert len(result.records) == 1
    r = result.records[0]
    assert r.contact_call == "K4JJQ"
    assert r.submitted_by == "KC0UJC"
...
def test_adif_master_format_with_pota_and_digital():
    ...
    content = (
        "<ADIF_VER:5>3.1<EOR><EOH>\n\n"
        '<CALL:4>NF0T <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>033300 '
...
def test_adif_master_format_no_blank_line_after_eoh():
    ...
    content = (
        ...
        '<CALL:5>K4JJQ <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>032800 <EOR>\n'
        '<CALL:4>NF0T <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>033300 <EOR>\n'
    )
...
def test_adif_master_format_with_special_chars_in_name():
    ...
    content = (
        "<ADIF_VER:5>3.1<EOR><EOH>\n\n"
        '<CALL:5>K4JJQ <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>032800 '
        '<NAME:21>James A. "Jim" Howard <EOR>\n'
    )
...
def test_adif_master_format_with_header_text():
    ...
    content = (
        ...
        "Copyright (C) 2005 - 2024 UU0JC, DXShell.com\n"
...
def test_adif_master_format_real_world_example():
    ...
    content = (
        ...
        '<CALL:5>K4JJQ <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>032800 '
        '<RST_RCVD:3>+06 <RST_SENT:3>+03 <STATION_CALLSIGN:6>KC0UJC <OPERATOR:6>KC0UJC <GRIDSQUARE:4>FM18 '
        '<MY_GRIDSQUARE:6>EN32ia <MY_STATE:2>IA <NAME:13>John J. Quinn <DXCC:3>291 <QTH:9>Warrenton '
        ...
        '<CALL:4>W0JW <MODE:2>CW <BAND:2>6m <FREQ:6>50.100 <QSO_DATE:8>20260609 <TIME_ON:6>041259 '
        '<RST_RCVD:3>229 <RST_SENT:3>559 <STATION_CALLSIGN:6>KC0UJC <OPERATOR:6>KC0UJC <GRIDSQUARE:6>EN31be '
        '<MY_GRIDSQUARE:6>EN32ia <MY_STATE:2>IA <NAME:14>John K. Fickes <DXCC:3>291 <QTH:5>Truro '
        '<STATE:2>IA <CQZ:1>4 <ITUZ:1>7 <QSLMSG:12>POTA US-9913 <MY_SIG:4>POTA <MY_SIG_INFO:7>US-9913 '
        '<MY_POTA_REF:7>US-9913 <MY_RIG:7>FT-450d <MY_COUNTRY:3>USA <MY_CNTY:8>IA,Story '
        '<COMMENT:25>20260609 POTA ACT US-9913 <TX_PWR:2>50 <EOR>\n'
        
        '<CALL:5>W6GRE <MODE:3>FT8 <BAND:2>6m <FREQ:6>50.313 <QSO_DATE:8>20260609 <TIME_ON:6>033700 '
        '<RST_RCVD:3>-11 <RST_SENT:3>-15 <STATION_CALLSIGN:6>KC0UJC <OPERATOR:6>KC0UJC <GRIDSQUARE:4>EN31 '
        '<MY_GRIDSQUARE:6>EN32ia <MY_STATE:2>IA <NAME:21>James A. "Jim" Howard <DXCC:3>291 <QTH:9>Polk City '
